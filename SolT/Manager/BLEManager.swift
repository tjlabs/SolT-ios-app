...
class BLEManager: NSObject, CBCentralManagerDelegate, CBPeripheralDelegate {
    let BLE_VALID_TIME: Double = 1000
    ...
    var bleStronggestID: String = ""
    var bleStronggestRSSI: Int = Int.min
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        ...
        if let bleName = discoveredPeripheral.name {
            if bleName.contains("TJ-") || bleName.contains("NI-") {
                ...
                let validTime = BLE_VALID_TIME*2
                ...
                if RSSI.intValue != 127 {
                    ...
                    let rssiValueInt = RSSI.intValue
                    if (bleScanned.contains(where: condition)) {
                        if rssiValueInt > bleStronggestRSSI {
                            bleStronggestID = bleName
                            bleStronggestRSSI = rssiValueInt
                        }
...
//        self.waitTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(self.waitTimerUpdate), userInfo: nil, repeats: true)
    ...
    public func getStronggestBLE() -> (String, Int) {
        return (bleStronggestID, bleStronggestRSSI)
    }
